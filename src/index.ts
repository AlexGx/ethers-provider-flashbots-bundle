...
export class BuilderBroadcaster extends FlashbotsBundleProvider {
  ...
  constructor(genericProvider: Provider, authSigner: Signer , network: Networkish, connectionInfoOrUrls: Array<FetchRequest>) {
    const defaultConnectionInfo: ConnectionEndpoint = DEFAULT_FLASHBOTS_RELAY
   ...
   */
   static async createBroadcaster(
    ...
    const connectionInfoOrUrlArray: FetchRequest[] = Array.isArray(builderEndpoints)
    ? builderEndpoints.map((url: ConnectionEndpoint) => {
      const connect: FetchRequest = typeof url === 'string' ? new FetchRequest(url) : url.clone()
      connect.retryFunc = FlashbotsBundleProvider.throttleCallback
      return connect
    })
    : [];
  ...
  public async broadcastBundle(
    ...
    // todo: add errors //return {error: {message: response.error.message, code: response.error.code}}
  ...
  private async requestBroadcast(body: string) {
    ...
    for (let connectionInfo of this.connectionInfoArr) {
      ...
      request.setHeader('Content-Type', 'application/json')
      request.setHeader('X-Flashbots-Signature', signature)
      request.body = body
      ...
      const promise = new Promise(async (resolve, reject) => {
        // resolve(await request.send())
        const resp = await request.send()
        try {
          resp.assertOk()
        } catch (err) {
          // throw new Error(`Request failed with status ${resp.statusCode} (URL: ${request.url}): ${resp.bodyText}`)
          reject(new Error(`Request failed with status ${resp.statusCode} (URL: ${request.url}): ${resp.bodyText}`))
        }
        resolve(resp.bodyJson)
      });
      responseHandles.push(promise);
    }

    return await Promise.all(responseHandles)
  }
} 
